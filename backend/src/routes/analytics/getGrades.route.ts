...
export const getGrades = async (queryBody: QueryPayload, firebase_uid: string) => {
    ...
    for (const quizAttempt of enrolment.quizAttempts) {
        ...
        const allAnswered = quizAttempt.responses.every((attempt) => {
            if (attempt.marked && attempt.mark === 0) {
                incorrectTags.push(attempt.question.tag);
            }
        ...
        if (allAnswered) {
            quizGrade.marksAwarded = quizAttempt.mark;
